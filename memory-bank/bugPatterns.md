...
### 2026-08-15: "Invalid authentication credentials." thi thoảng hiện ở `/quotes` — access token JWT hết hạn nhưng SPA không có cơ chế tự refresh-on-401

- Area: Frontend auth token lifecycle (`frontend/src/api/http.ts`, `frontend/src/stores/auth.store.ts`, `frontend/src/composables/useQuotesPage.ts`)
- Trigger: Người dùng để trang `/quotes` mở liên tục quá `ACCESS_TOKEN_EXPIRE_MINUTES` (mặc định 30 phút) mà không F5 lại — thao tác tiếp theo (đổi trang/sắp xếp/lọc) nhận lỗi 401 hiện nguyên văn "Invalid authentication credentials." (detail tiếng Anh từ backend, xem `backend/app/auth/dependencies.py`).
- Root cause: `authStore.initialize()` chỉ gọi `/auth/refresh` ĐÚNG 1 LẦN lúc load trang; sau đó access token cố định cho tới lần F5 tiếp theo — không có timer làm mới định kỳ, không có interceptor bắt 401. Frontend cũng hiện thẳng `error.message` (chính là `detail` từ backend) lên UI không dịch. Phụ: `QuotesPage.vue` từng truyền `authStore.accessToken` vào `useQuotesPage()` dưới dạng chuỗi tĩnh (chụp 1 lần lúc mount) thay vì đọc "sống" mỗi lần gọi.
- Fix: Thêm cơ chế retry-on-401 dùng chung ở tầng `http.ts` (`setUnauthorizedHandler`, tách `performRequest`/`apiRequest`), action `ensureFreshAccessToken()` ở auth store (dedupe nhiều 401 đồng thời qua 1 `refreshPromise` chung — tránh rotate refresh-token cookie nhiều lần cùng lúc, cùng loại rủi ro với race Ctrl+R đã ghi ở trên), đăng ký handler trong `main.ts` (refresh thất bại thật thì `router.push` về `/login`), đổi `useQuotesPage` sang nhận `getAccessToken: () => string | null`, dịch lỗi 401 còn sót lại sau retry thành "Phiên đăng nhập đã hết hạn, vui lòng tải lại trang."
