...
def build_user(permission_codes: list[str]) -> User:
    ...
    role = Role(id=uuid4(), name="catalog_importer", is_system=False)
...
def override_dependencies(app: FastAPI) -> Generator[MockJobAdminService, None, None]:
    ...
    app.dependency_overrides[get_current_user] = lambda: build_user(
        ["material_types.import", "materials.import", "suppliers.import"],
    )
...
@pytest.mark.asyncio
async def test_download_catalog_import_template_requires_entity_permission(
    app: FastAPI,
    client: AsyncClient,
) -> None:
    app.dependency_overrides[get_current_user] = lambda: build_user(["materials.import"])

    allowed_response = await client.get("/api/v1/catalog-imports/templates/materials")
...
@pytest.mark.asyncio
async def test_start_catalog_import_creates_job_and_enqueues_after_commit(
    client: AsyncClient,
    ...
    files = {
        "file": (
            "material_types.csv",
            b"code,name,status,note\nNL,Nguyen lieu,active,",
            "text/csv",
        ),
    }

    response = await client.post("/api/v1/catalog-imports/material_types", files=files)

    assert response.status_code == 201
    data = response.json()
    assert data["entity_type"] == "material_types"
...
@pytest.mark.asyncio
async def test_download_catalog_import_error_file_uses_job_entity_permission(
    client: AsyncClient,
    override_dependencies: MockJobAdminService,
) -> None:
    job = await override_dependencies.create_import_job(
        file_id=uuid4(),
        user_id=uuid4(),
    ...
    job.failed_rows = 1
    job.errors_json = [{"row": 2, "code": "SUP-01", "errors": ["Tên NCC là bắt buộc."]}]

    response = await client.get(f"/api/v1/catalog-imports/{job.id}/error-file")

    assert response.status_code == 200
    assert "SUP-01" in response.text
    assert "Tên NCC là bắt buộc." in response.text
